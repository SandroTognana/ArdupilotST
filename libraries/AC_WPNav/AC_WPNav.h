...
#define WPNAV_ENGAGE_SEC		         5.0f        // default engage time for loiter
#define WPNAV_LOITER_MODE				0			// default loiter standard; 1=ALT_HOLD+LOITER
#define WPNAV_LOITER_DB					30			// default ALT_HOLD+LOITER commutation threshold
...
#define BRAKE_RATE 10.0f                     //set it from 5 to 20, means the number of deg/s the copter rolls/tilt during braking
...
public:
    ...
    /// shared methods
    ///
	AP_Float    _loiter_engage_sec;      // 
	AP_Int16  	loiter_deadband;		// loiter to alt_hold threshold switch
	AP_Float    brake_rate;				// set it from 5 to 20, means the number of deg/s the copter rolls/tilt during braking
    AP_Int16    max_braking_angle;		// set it from 2000 to 4500 in centidegrees
	...
	//uint8_t get_loiter_deadband() {return _loiter_deadband;};
...
protected:
    ...
    float       _loiter_leash;          // loiter's horizontal leash length in cm.  
										// used to stop the pilot from pushing the target location too far from the current location
