...
#define AN4 58
#define AN5	59
#define COPTER_LED_1 AN4
#define COPTER_LED_2 AN5
...
const AP_Param::GroupInfo AC_WPNav::var_info[] PROGMEM = {
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("LOIT_DB",    8, AC_WPNav, loiter_deadband, WPNAV_LOITER_DB),
    ...
    // @Increment: 1
    // @User: Standard
    AP_GROUPINFO("BR_RATE",    9, AC_WPNav, brake_rate, BRAKE_RATE),
    ...
    // @Increment: 100
    // @User: Standard
    AP_GROUPINFO("BR_MAX_ANG",   10, AC_WPNav, max_braking_angle, MAX_BRAKING_ANGLE),
	
    // @Param: SPD_MAX_BR
    // @DisplayName: the speed (cm/s) that requires max braking angle
	// #Description: 
	// @Suggested range: 
    // @Units: cm/s
    // @Range: 
    // @Increment: 
    // @User: Standard
    AP_GROUPINFO("BR_MAX_SPD",   11, AC_WPNav, speed_max_braking, SPEED_MAX_BRAKING),
    ...
    // @Increment: 
    // @User: Standard
    AP_GROUPINFO("BR_SPEED_0",   12, AC_WPNav, speed_0, SPEED_0),
...
void AC_WPNav::get_stopping_point(const Vector3f& position, const Vector3f& velocity, Vector3f &target) const
{
	...
	target_dist = constrain_float(target_dist, 0, _wp_leash_xy*2.0f);  // orig.  _wp_leash_xy*2.0f 
...
{
    // convert pilot input to desired velocity in cm/s NOTA: è una accelerazione!
   _pilot_vel_forward_cms = -control_pitch * _loiter_accel_cms / 4500.0f;	// normalizza rispetto ai 45 gradi
...
void AC_WPNav::translate_loiter_target_movements(float nav_dt)
{
	...
	// add desired change in velocity to current target velocity (usando l'accelerazione richiesta)
...
void AC_WPNav::update_loiter()
{
    ...
    switch (_loiter_step) {
        case 0:
			...
			// input: _pilot_vel_forward_cms, _pilot_vel_right_cms (orientate rispetto alla macchina)
			// le ruota rispetto a lat e lon
			// incrementa/decrementa la velocità e la limita (_loiter_speed_cms)
			// aggiorna la posizione del target (_target.x e _target.y) in funzione della velocità
			// output: _target.x, _target.y e target_vel.x, target_vel.y
        ...
        case 1:
			...
			// input: _target.x e _target.y
			// calcola l'errore tra posizione attuale e target
			// calcola la distanza tra posizione attuale e target (lunghezza errore)
			// calcola il punto che determina se usare sqrt(velocità) o velocità lineare: al di sopra di due volte linear_distance = _wp_accel_cms/(2.0f*kP*kP) va con la radice quadrata, al di sotto, va lineare
			// questa kP è la position kP
			// limita la velocità a max_speed_cms (WPNAV_LOITER_SPEED_MAX_TO_CORRECT_ERROR)
			// aggiunge alla velocità del controllo di posizione (desired_vel) quella del target: desired_vel.x += _target_vel.x;        desired_vel.y += _target_vel.y;
			// output: desired_vel.x e desired_vel.y (sempre orientate in lat/lon)
        ...
        case 2:
			...
			// input: desired_vel.x e desired_vel.y
			// calcola l'accelerazione che serve come differenza tra vel desiderata e vel precedente (memorizzata localmente) diviso il delta t
			// calcola l'errore di velocità tra desired.vel e velocità istantanea reale e lo da in pasto al controller PID (rate pid)
			// somma all'accelerazione che serve calcolata prima, l'uscita del PID
			// limita l'accelerazione a WPNAV_ACCEL_MAX (9.8m/s^2)
			// output: desired_accel x e y
        ...
        case 3:
			...
			// input: desired_accel (orientate in lat,lon)
			// ruota le componenti di desired accel da lat lon al frame (roll, pitch) accel_right/forward
			// calcola _desired_roll e _desired_pitch (angoli) come arcotangente(accel_right/forward)
			// limita desired_roll, desired_pitch secondo _lean_angle_max_cd, _lean_angle_max_cd
			...
			if (cont>10) 
			{
				...
				hal.gpio->write(COPTER_LED_1,stato);
			}
...
/*
Had the same issue with APM (trunk) before parametrizing the stopping "target_dist" multiplier 2.0f  (wp_leash_xy*2.0f),  line 150 of AC_WPNav.cpp.
i.e., AC_WPNav.cpp mods:  
          <line 63:>
            ~~~
...
#define WPNAV_WP_STPG_LSH                1.0f        // default stopping-strength leash rate reduced from 2.0f
            ~~~
          <inserted at line 42:>
...
This adds Stopping XYLeash Rate parameter to the "full parameters..." defaulted to 1.. What works for me though to get a really crisp locked in stopping power is currently .7, ..the current 2.0f (hard coded) feels wishy washy (just me;o).
...
void AC_WPNav::get_loiter_position_to_velocity(float dt, float max_speed_cms)
{
    ...
    if (kP <= 0.0f) {
        ...
        if( dist_error_total > 2.0f*linear_distance ) {
            ...
            desired_vel.x = kP * dist_error.x;
            desired_vel.y = kP * dist_error.y;
        }
